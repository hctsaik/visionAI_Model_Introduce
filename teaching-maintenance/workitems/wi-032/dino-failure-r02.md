...
Keep this exact three-part composition and the SAME bolt and washer. Revise labels only, not the scene: both left panels contain image PLUS candidate boxes so label them「完整可見：影像＋候選框」「夾具遮擋：影像＋候選框」. Remove hex-head screw from black occluder; make it smooth black. Right counts refer to「待數零件」 not every physical object. Title center output「假設漏檢示意」. Explicit「非模型實測」. All other instructional meaning retained.
