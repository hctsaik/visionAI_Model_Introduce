...
def _clean_engineering_slides(value: Any, label: str) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(value):
        ...
        if not isinstance(item, dict) or not required <= set(item) or set(item) - required - {"evidence_links"}:
...
def page_payload(
    ...
    if revised:
        ...
        return {
            "index": page_index + 1, "label": label,
...
def make_course_data() -> dict[str, Any]:
    ...
    for topic in topics:
        ...
        topic.update(takeaway=story["promise"]["body"], failure=story["mental_model"]["limit"], selection=story["selection"])
        locks = [
