...
## r02 修正
Rebuild the LightGlue diagram into cleaner geometry with SAME simple flat machined metal PLATE seen top-down in BOTH images (one rotates only slightly in-plane). Plate has ONE unique upper-right notch and TWO round holes along lower portion, left hole B and right hole C. A always at inner notch corner. There must be NO bent vertical walls or holes on different planes. Only one major before/after candidate comparison in center. B on left image initially has two dashed orange links to B and C in right image; after using stable A-A notch context, keep blue B-B and terminate gray B-C. The physical points NEVER move to another edge or another hole; retain same coordinates of A B C throughout the illustrated pair. Left region upstream extractor produces these points and descriptor strips. Right region output shows the same pair with A-A/B-B/C-C, then a clearly separate geometry solver label; remove the lower redundant 3D bracket and homography grid. Keep learned confidence/pruning as one gray unmatched point outside the plate canceled in candidate SET, don't remove an actual hole or source pixel. Large labels, fewer paragraphs, no performance promise. Preserve headline and yellow takeaway. Three major stages, arrows meaningful, white/pale-blue engineering reference aesthetic.
