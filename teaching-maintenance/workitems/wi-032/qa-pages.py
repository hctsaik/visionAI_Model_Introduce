...
slugs=sys.argv[1:] or list(json.loads((W/'lesson-content.json').read_text(encoding='utf-8')))
  ...
  for route in ['interactive-learning.html','docs/index.html']:
   ...
   for width,height in [(1440,1000),(360,800)]:
    ...
    try:
     ...
     assert p.locator('.beginner-visual').count()==3
     ...
     for i,card in enumerate(p.locator('.beginner-visual').all(),1):
      ...
      im=fig.locator('img:visible');assert im.count()==1;im.evaluate('(i)=>i.decode()')
      state=im.evaluate('(i)=>({src:i.currentSrc,nw:i.naturalWidth,nh:i.naturalHeight,box:i.getBoundingClientRect().toJSON()})')
      assert 'wi032-' in state['src'];assert state['box']['width']<=width
      ...
      p.locator('#lightbox img').evaluate('(i)=>i.decode()');p.keyboard.press('Escape');assert not p.locator('#lightbox[open]').count();row['zoom_checks']+=1
     ...
     for i,fig in enumerate(figs.all(),1):
      fig.scroll_into_view_if_needed();im=fig.locator('img');im.evaluate('(i)=>i.decode()')
      state=im.evaluate('(i)=>({src:i.currentSrc,nw:i.naturalWidth,nh:i.naturalHeight,box:i.getBoundingClientRect().toJSON()})')
      assert 'WI032-' in state['src'];assert ('mobile' if width==360 else 'desktop') in state['src']
      ...
      p.locator('#lightbox[open]').wait_for();p.locator('#lightbox img').evaluate('(i)=>i.decode()')
