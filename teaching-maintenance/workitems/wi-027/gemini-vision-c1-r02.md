# WI-027 Gemini Vision：把影像與問題交給多模態服務 r02
...
Only correction: node 2 generic capability Gemini Vision is NOT an actual API model identifier. Replace 模型：Gemini Vision（多模態） with 指定可用模型 ID. Keep image+question → closed API → answer示意 and exact right empty socket evidence. Add original-image location box on right socket for crop. No invented model ID or private architecture.
