# WI-027 DINOv2 與 DINOv3：比較的是整套用途 r02
...
Fix query path and identity: same single held-out bracket, same holes/shape throughout all panels. Each DINOv2/DINOv3 lane uses its OWN encoder twice logically: reference images → that encoder → reference features → own reference database, and query image → that SAME encoder (may draw second same-labelled box) → queryfeature → explicit 檢索 matching against database → candidate outputs. Query must not bypass encoder and must not be inserted into reference database. Maintain exact 3 main groups. No changed bracket shape between original and query. Use only three candidate examples to reduce density; quality/cost axes are questions no invented wins.
