...
def save(): (OUT/'recheck-browser-coverage.json').write_text(json.dumps(ledger,ensure_ascii=False,indent=2),encoding='utf8')
def images(p):
...
def screenshot(p,name):
 p.screenshot(path=str(OUT/('recheck-'+name+'.png')));ledger['screenshots'].append('recheck-'+name+'.png')
def inspect(p):
...
try:
 with sync_playwright() as pw:
  ...
  for width in [1440,390]:
   ctx=b.new_context(viewport={'width':width,'height':900},reduced_motion='reduce');p=ctx.new_page();p.set_default_timeout(10000);errors=[];p.on('pageerror',lambda e:errors.append(str(e)))
