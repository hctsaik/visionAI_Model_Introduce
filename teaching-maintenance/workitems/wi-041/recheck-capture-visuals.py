...
with sync_playwright() as pw:
 ...
 for w in [1440,390]:
  ...
  for t in d['topics']:
   ...
   e=p.locator('main [data-action="open-concept"]').first;e.scroll_into_view_if_needed();p.locator('main img').evaluate_all('async es=>{es.forEach(e=>e.loading="eager");await Promise.all(es.map(e=>e.decode().catch(()=>{})))}');p.screenshot(path=str(O/f'recheck-visual-{w}-{t["id"]}.png'))
