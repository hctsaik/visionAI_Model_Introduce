...
OUT=Path(__file__).parent;URL='http://127.0.0.1:8000/docs/index.html';WIDTH=int(sys.argv[1]);FILE=OUT/f'recheck-coverage-{WIDTH}.json'
ledger={'started':time.strftime('%Y-%m-%d %H:%M:%S'),'width':WIDTH,'lessons':[],'routes':[],'screenshots':[],'prior_attempts':[],'fatal':None}
for oldfile in [OUT/'recheck-browser-coverage.json',FILE]:
 if oldfile.exists():
...
def snap(p,name):p.screenshot(path=str(OUT/('recheck-'+name+'.png')));ledger['screenshots'].append('recheck-'+name+'.png')
