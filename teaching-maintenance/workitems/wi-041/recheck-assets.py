...
for group,urls in [('public_assets',[BASE+urllib.parse.quote(r.as_posix(),safe='/') for r in assets]),('external_sources',external)]:
 ...
 (W/f'recheck-{group}.json').write_text(json.dumps(rows,ensure_ascii=False,indent=2),encoding='utf8')
...
(W/'recheck-asset-audit.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf8');print('Complete',json.dumps({'equal':result['public_equals_local'],'local_asset_failures':sum(not r['equal'] for r in local),'counts':result['counts']},ensure_ascii=False),flush=True)
