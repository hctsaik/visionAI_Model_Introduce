...
def save(): (OUT/'browser-mobile-coverage.json').write_text(json.dumps(ledger,ensure_ascii=False,indent=2),encoding='utf8')
def images(p):
  ...
  for width in [390]:
   ...
   for topic in topics:
    ...
    try:
     ...
     reveal=p.locator('summary').filter(has_text='?圾???舀???')
