...
with sync_playwright() as pw:
 ...
 for slug,a in content.items():
  for width in [1440,360]:
   ...
   assert not errors
   records.append({'slug':slug,'width':width,'core_comparison_and_cost_visible':True,'first_and_evidence_visible':True,'three_core_callouts':True,'images':images.evaluate_all('(xs)=>xs.map(i=>({src:i.currentSrc,width:i.getBoundingClientRect().width,height:i.getBoundingClientRect().height}))'),'overflow':False,'errors':errors})
   page.close()
 b.close()
