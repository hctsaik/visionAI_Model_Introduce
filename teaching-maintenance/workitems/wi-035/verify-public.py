...
if actual!=expected:print('Public deployment pending:',actual,'expected:',expected);sys.exit(3)
with sync_playwright() as p:
 ...
 for width,height in [(1440,1000),(390,844)]:
  ...
  page.goto(url+'#view=poc');page.locator('#poc-task').fill('公開新版操作核對')
  ...
  page.locator('.poc-step-nav button').nth(2).click();page.locator('#poc-owner').fill('品保')
  page.locator('[data-action="poc-preview"]').filter(visible=True).first.click();assert '公開新版操作核對' in page.locator('#poc-preview').inner_text()
  assert page.evaluate('document.documentElement.scrollWidth<=innerWidth');assert not errors
  page.evaluate('document.activeElement.blur();window.scrollTo(0,0)');page.screenshot(path=str(W/f'public-final-{width}.png'),full_page=True);context.close()
 b.close()
