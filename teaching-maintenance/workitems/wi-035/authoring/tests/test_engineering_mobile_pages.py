...
class EngineeringMobileTests(unittest.TestCase):
    ...
    def test_authored_label_survives_and_unknown_fields_still_fail(self):
        pages = copy.deepcopy(self.pages)
        pages[2]['label'] = '訓練時如何學會區分'
        cleaned = builder._clean_engineering_slides(pages, 'test')
        result = builder.page_payload({}, {}, 2, ('manifest.png', 'active/manifest.png'), {'engineering_slides': cleaned})
        self.assertEqual(result['label'], '訓練時如何學會區分')
        pages[2]['accidental_field'] = 'must not silently pass'
        with self.assertRaises(ValueError):
            builder._clean_engineering_slides(pages, 'test')
