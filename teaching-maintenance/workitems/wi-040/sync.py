...
for p in W.iterdir():
    if p.is_file() and p.suffix in {'.md', '.json', '.py', '.txt', '.cjs'}:
        files['workitems/wi-040/' + p.name] = p
for p in (C/'workitems/wi-039').iterdir():
    if p.is_file() and p.suffix in {'.md','.json','.py','.txt'}:
        files['workitems/wi-039/' + p.name] = p
...
for rel in ['_course_content/poc-decision.js', '_course_content/poc-workbench.js', '_course_content/poc-workbench.css', 'tools/build_interactive_learning_html.py', 'tests/test_poc_decision.cjs', 'tests/test_poc_workbench.py','tests/test_release_hardening.py','_course_content/topics/convnext.json','_course_content/topics/v-jepa.json','_course_content/topics/ad-anomalygpt.json']:
    files['workitems/wi-040/authoring/' + rel] = C/rel
missing = []
