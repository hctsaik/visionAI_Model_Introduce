...
- lesson objective: 需求改變時，資料、輸出與驗收也要改。
- page type: D — 按本圖資料處理或同条件比較安排。
- primary reading path: 具體工件／問題 → 目前：只有正常影像 → 先篩查：找可疑位置 → 改要面積：輪廓與校正
...
- pale-yellow takeaway: 需求改變時，資料、輸出與驗收也要改。 #FFF4CC
- major visual nodes:
  1. 目前：只有正常影像；具體視覺 deep-data-train
  2. 先篩查：找可疑位置；具體視覺 deep-output-location
  3. 改要面積：輪廓與校正；具體視覺 deep-transfer-area

## 機制、證據與修正

重排深讀8手機；仍是同一四孔板與下中線痕。只有正常品可先比較EfficientAD/PatchCore作可疑位置篩查；若需求變為定義刮傷面積，補輪廓標註及分割验证，實體面積再需校正/誤差。條件不適合可換方法，不沿用熱圖當精密輪廓。
