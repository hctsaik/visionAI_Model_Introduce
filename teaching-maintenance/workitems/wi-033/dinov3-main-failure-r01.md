...
- lesson objective: 同一缺口先看輸入，再看特徵與下游漏檢。
- page type: C — 按本圖資料處理或同条件比較安排。
- primary reading path: 具體工件／問題 → 原圖：右側缺口可見 → 粗取樣：細節變弱 → 重新取像後再驗證
...
- pale-yellow takeaway: 同一缺口先看輸入，再看特徵與下游漏檢。 #FFF4CC
- major visual nodes:
  1. 原圖：右側缺口可見；具體視覺 dino-notch
  2. 粗取樣：細節變弱；具體視覺 dino-resolution
  3. 重新取像後再驗證；具體視覺 dino-retake

## 機制、證據與修正

替換DINOv3原反例工件切換及手机小字。三段使用同一右側有缺口墊圈；取樣格為分析式SVG示意，不冒充真實模型輸出。保留原桌機核心與方法比較。
