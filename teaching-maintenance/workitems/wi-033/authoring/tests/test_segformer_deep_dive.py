"""Focused contract and browser QA for the SegFormer eight-chapter deep dive."""
...
TOPIC_PATH = ROOT / "_course_content" / "topics" / "segformer.json"
...
def edge_executable() -> str:
    ...
    if not found:
        raise FileNotFoundError("Microsoft Edge is required for SegFormer browser QA")
    return found


class SegFormerDeepDiveTests(unittest.TestCase):
    ...
    def test_runtime_renders_only_the_new_deep_dive_and_supports_slide_eight(self) -> None:
        ...
        with sync_playwright() as playwright:
            ...
            page.goto(
                HTML_PATH.resolve().as_uri() + "#view=lesson&lesson=segformer&slide=8",
                wait_until="load",
            )
            page.wait_for_selector('[data-deep-dive="segformer"]')
            ...
            deep = page.locator('[data-deep-dive="segformer"]')
            ...
            mobile.goto(
                HTML_PATH.resolve().as_uri() + "#view=lesson&lesson=segformer&slide=4",
                wait_until="load",
            )
            mobile.wait_for_selector('[data-deep-dive="segformer"]')
