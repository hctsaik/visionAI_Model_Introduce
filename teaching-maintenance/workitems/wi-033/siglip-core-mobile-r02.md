...
- lesson objective: 每對各算損失，部署仍須驗證候選排名。
- page type: C — 按本圖資料處理或同条件比較安排。
- primary reading path: 具體工件／問題 → 訓練：圖文身份決定正負 → 逐對損失，共同更新參數 → 部署：編碼後比較候選
...
- pale-yellow takeaway: 每對各算損失，部署仍須驗證候選排名。 #FFF4CC
- major visual nodes:
  1. 訓練：圖文身份決定正負；具體視覺 paired-training
  2. 逐對損失，共同更新參數；具體視覺 siglip-local
  3. 部署：編碼後比較候選；具體視覺 siglip-match
