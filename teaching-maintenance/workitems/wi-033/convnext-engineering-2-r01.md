# ConvNeXt：先看鄰域，再組合不同線索
- lesson objective: 空間混合不跨通道，通道混合在同位置組合。
- page type: C — 真實資料變換用C，同條件比較用D。
- primary reading path: 每通道各看空間鄰域 → 同位置混合各通道 → 殘差保留原本表示 → 空間混合不跨通道，通道混合在同位置組合。
...
- pale-yellow takeaway: 空間混合不跨通道，通道混合在同位置組合。 #FFF4CC
- major visual nodes:
  1. 每通道各看空間鄰域；證據場景 b3-cnx-spatial
  2. 同位置混合各通道；證據場景 b3-cnx-channel
  3. 殘差保留原本表示；證據場景 b3-cnx-residual

## 輸入、方法、輸出與證據
原始區塊用7×7 depthwise卷積，各通道獨立混合鄰域；之後LayerNorm、通道擴張、GELU、通道縮回及layer scale，再加直通路。圖用小鄰域和兩通道示意角色，不聲稱展示完整實際張量。
