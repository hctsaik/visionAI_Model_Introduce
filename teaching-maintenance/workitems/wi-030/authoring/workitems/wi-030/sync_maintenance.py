...
p=S/'manifest.json';m=json.loads(p.read_text(encoding='utf-8'));m['workitem']='WI-030'
new={}
for src in W.glob('qa-*/report.json'):
 new['workitems/wi-030/'+src.relative_to(W).as_posix()]=src
for src in list(W.glob('*.md'))+list(W.glob('*.json')):
 new['workitems/wi-030/'+src.name]=src
for src in W.glob('final-page-reading-*.txt'):
 new['workitems/wi-030/'+src.name]=src
for name,src in [('WORKITEMS.md',R/'WORKITEMS.md'),('BEGINNER_VISUAL_TODO.md',C/'BEGINNER_VISUAL_TODO.md'),('BEGINNER_VISUAL_STATUS.md',C/'BEGINNER_VISUAL_STATUS.md')]:new['workitems/wi-030/'+name]=src
for slug in json.loads((W/'lesson-content.json').read_text(encoding='utf-8')):
 src=C/'_course_content/topics'/f'{slug}.json'
 new['workitems/wi-030/authoring/_course_content/topics/'+src.name]=src
 t=json.loads(src.read_text(encoding='utf-8'));rel=t['review_trace']['authority']
 new['workitems/wi-030/authoring/'+rel]=C/rel
for rel in ['_course_content/learner-briefs.json','tools/build_interactive_learning_html.py','tools/build_github_pages_site.py','tests/test_tall_mobile_artwork.py','tests/test_collapsed_deep_dive.py','tests/test_github_pages_bundle.py']:
 new['workitems/wi-030/authoring/'+rel]=C/rel
for src in W.glob('*.py'):new['workitems/wi-030/authoring/workitems/wi-030/'+src.name]=src
