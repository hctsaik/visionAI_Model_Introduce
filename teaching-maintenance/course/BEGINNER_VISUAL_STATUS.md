...
## WI-028 八個時序題開始（2026-09-10）
- 使用者以截圖明確要求八題製作；範圍／checklist：`workitems/wi-028/PLAN.md`。
- 已完成：工作入口與技能、五份權威適用段落核對；Git 乾淨。
- 即將執行：保存八題基準、實看認可參考與查第一手來源，建立 preflight／代表原型。
- 生成模式：built-in imagegen，示意非模型實測；prompt／版本／候選保存在 workitems/wi-028/。目前尚未生成、整合、評分或測試，使用者核准 pending。
- WI028 checkpoint：16張基準與八題來源已保存；frame-c1-r01-desktop.png 已生成實看，照片位移與簡化重疊條件不一致，未通過。下一步r02校正同一方塊小位移及手機重排；未整合／測試、使用者核准pending。證據workitems/wi-028/prototype-review.md。
- WI028原型完成：r02桌面／手機及936／328px實看，自評95／94；18故事設計、17新preflight通過。即將built-in imagegen生成其餘圖，逐圖另審，prompt／PNG／meta在workitems/wi-028/，未整合／測試、user approval pending。
- WI028生成checkpoint：bg／LK／RAFT桌面r01已保存；LK同板箭頭方向不一致需r02，未採用。背景及RAFT待尺寸／手機。八課完整文字草稿已保存lesson-content.json，未整合／測試。接續review-progress.md。
- WI028影片核心checkpoint：ByteTrack／ConvLSTM桌面r01已實看主要責任成立；VideoMAE遮蔽透明與token路徑、V-JEPA逐幀編碼誤讀需r02。LK r02和四張傳統法反例／比較仍生成中。正式topic未改，測試未跑，user approval pending。
- WI028手機修正checkpoint：LK r03遮擋點已修；VideoMAE r04需移除新增圓章及恢復不透明遮蔽；比較mobile背景參考仍有工件須刪；ByteTrack預測框重疊需加強。正式未整合，驗證未跑；詳細review-progress.md，使用者核准pending。
WI-028 checkpoint: 36 final desktop/mobile PNGs actually reviewed at native and 936/328 widths; corrections and exact prompts retained in workitems/wi-028. Selected manifest and eight topic/model/learner integrations completed. Build in progress; web QA not yet run, public update not yet published. All user approval fields pending. Preflight selection must exclude image-review.md/prototype-review.md, which are review logs, not briefs.
...
## WI-030 啟動
四連結與截圖八題共12課重作，基準12fc9ff乾淨；已核對技能和五份權威適用規則。尚未產圖或整合，下一步保存基準與原型設計。檔案workitems/wi-030/PLAN.md；測試未跑、使用者核准pending。

## WI-030 原型 checkpoint
已保存12課、24桌機手機基線與第一手來源；21故事preflight通過。PatchCore r01因箭頭與序號未通過，r02已生成並原生實看，手機獨立重排即將生成；生成模式built-in imagegen，prompt與PNG保存在workitems/wi-030。尚未啟用新版、未跑本輪頁面測試、使用者核准pending。下一步手機實看及其他家族原型。

## WI-030 內容與程式 checkpoint
十二課正文、自測與工作比較已寫入workitems/wi-030/lesson-content.json；未改正式topic。builder增加可選deep_dive.default_collapsed並區分工程／舊章導覽；型別和保留章節聚焦測試1 passed＋4 subtests，頁面測試待生成整合。圖片原型仍修正，具體失败與改法見prototype-review.md。下一步完成圖像審查，再整合並測預設主線。

## WI-030 生成 checkpoint（2026-09-11）
已審選5故事：PatchCore、PaDiM、ResNet、U-Net核心與AD光照反例；selected-assets.json只記真正看過且達標的候選。尚未正式啟用。其餘圖在原型／手機修正或生成中，全部失敗版本保留；最新細節prototype-review.md。手機328px實看發現EfficientAD雙欄和AD四方法比較字過小，改內部上下排列，不以放大代替首讀。待測尺寸檢查與整合未跑；工具型別檢查1＋4subtests仍是目前唯一程式測試結果。下一步完成候選、逐圖評分、12課整合與實頁驗證。

WI-030目前持續逐图審查，21故事42張目標尚未完成，未整合公開頁。姿態鏡射／K畸變／三點暗示已記錄並修訂，手機三欄改全寬。產物workitems/wi-030；頁面與HTTP測試未跑，使用者核准pending。

WI-030 checkpoint：15個故事30張已逐圖原生及936/328px審查，selected-assets為候選選定。先整合已齊圖的PatchCore、PaDiM、AnomalyDINO、EfficientAD、U-Net、SegFormer六課本機來源；未建置及發布。即將執行首輪builder驗證，輸出interactive-learning.html與docs。其餘六課待姿態、實例與分類比較修訂，不能視為全輪完成；使用者核准pending。
